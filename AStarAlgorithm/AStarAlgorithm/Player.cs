...
namespace Algorithim
{
    ...
    class Player
    {
        ...
        void AStar()
        {
            ...
            //G-시작점에서 해당좌표까지 이동하는데 드는 비용(작을수록 좋음,경로에 따라 달라짐)
            //H-목적지에서 얼마나 가까운지(작을수록 좋음, 고정)

            //(y,x) 방문했었는지 여부(방문=closed 상태)
            bool[,] closed = new bool[_board.Size, _board.Size];//closeList 배열사용하면 연산속도 빠름 메모리는 잡아먹음
            //(y,x)가는길을 한번이라도 발견했는지
            //발견x -MaxValue, 발견0- F=G+H
            ...
            while (pq.Count>0)
            {
                //제일 좋은 후보를 찾는다
                PQNode node = pq.Pop();
                //동일한 좌표를 여러경로로 찾아서, 더 빠른 경로로 인해 이미 방문(closed)된 경우 스킵
                ...
                if (node.Y == _board.DestY && node.X == _board.DestX)
                    break;
                //상하좌우 등 이동할 수 있는 좌표인지 확인해서 open 한다.
        ...
        void RightHand()
        {
            ...
            {
                // 1.바라보는 방향을 기준으로 오른쪽으로 갈수 있는지 확인
